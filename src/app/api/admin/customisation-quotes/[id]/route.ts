...
import { markCustomisationQuoteViewed, deleteCustomisationQuote } from "@/lib/admin/customisation-quotes";
...
export const PATCH = withErrorHandling(async (_request: Request, { params }: Params) => {
  ...
  await markCustomisationQuoteViewed(id);
...
export const DELETE = withErrorHandling(async (_request: Request, { params }: Params) => {
  ...
  const { id } = await params;
  await deleteCustomisationQuote(id);
  return NextResponse.json({ ok: true });
});
